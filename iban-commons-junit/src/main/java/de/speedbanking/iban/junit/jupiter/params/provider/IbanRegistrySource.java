...
public @interface IbanRegistrySource {
    ...
    class IbanRegistryArgumentsProvider implements ArgumentsProvider, AnnotationConsumer<IbanRegistrySource> {
        ...
        public Stream<? extends Arguments> provideArguments(ParameterDeclarations parameters, ExtensionContext context) {
            ...
            Predicate<IbanRegistry> attributePredicate;
            if (predicates.isEmpty() || config.filterMode() == FilterMode.ALL) {
                attributePredicate = predicates.stream().reduce(e -> true,  Predicate::and);
            } else {
                attributePredicate = predicates.stream().reduce(e -> false,  Predicate::or);
            }

            // Step 4: apply all filters
            List<IbanRegistry> result = Arrays.stream(IbanRegistry.values())
                .filter(i -> includeFilter == null || includeFilter.contains(i))
                .filter(attributePredicate)
                .filter(e -> excludeFilter == null || !excludeFilter.contains(e))
                .collect(toList());
