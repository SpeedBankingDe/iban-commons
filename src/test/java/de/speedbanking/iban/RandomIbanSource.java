...
public @interface RandomIbanSource {
    ...
    class RandomIbanArgumentsProvider implements ArgumentsProvider, AnnotationConsumer<RandomIbanSource> {
        ...
        private static List<IbanRegistry> validateAndBuild(RandomIbanSource config) {
            ...
            if (config.value() != null && config.value().length > 0 && config.includeCountries() != null && config.includeCountries().length > 0) {
            ...
            IbanRegistry[] includes = config.value() != null && config.value().length > 0
                ? config.value()
                : config.includeCountries() != null && config.includeCountries().length > 0
                    ? config.includeCountries()
                    : new IbanRegistry[0];
            ...
            if (config.excludeCountries() != null && config.excludeCountries().length > 0) {
                Set<IbanRegistry> excludeSet = EnumSet.copyOf(Arrays.asList(config.excludeCountries()));
                ...
                if (includes.length > 0) {
                    List<IbanRegistry> overlap = Arrays.stream(includes)
                        ...
                        .collect(toList());
                    if (overlap.size() > 1) {
                        throwIllegalArgument(overlap.size()
                            + " countries appear in both include and exclude: "
                            + overlap.stream().map(IbanRegistry::getCountryCode).collect(joining(", ")));
                    } else if (overlap.size() == 1) {
                        throwIllegalArgument("Country '" + overlap.get(0).getCountryCode() + "' appears in both include and exclude");
                    }
                }
            ...
            if (pool.isEmpty()) {
                throwIllegalArgument("the country pool is empty after applying the configured filters");
            }
