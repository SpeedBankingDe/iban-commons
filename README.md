...
An optional [`iban-commons-bankdata`](#bank-data-lookup-iban-commons-bankdata) module resolves the BIC and bank name behind an IBAN or a country-specific bank code (DE, AT, CH, CZ, BE), with a bundled offline fallback dataset and lazy background refresh.
...
Belgian bank code), self-hosted and open-source. Initial country coverage: **DE, AT, CH, CZ, BE**.
