...
final class BundledDataRefreshToolTest {
    ...
    void isHelpRequested_shortFlag_returnsTrue() {
        assertThat(BundledDataRefreshTool.isHelpRequested(new String[] {"-h"})).isTrue();
    }

    @Test
    void isHelpRequested_longFlag_returnsTrue() {
        assertThat(BundledDataRefreshTool.isHelpRequested(new String[] {"--help"})).isTrue();
    }

    @Test
    void isHelpRequested_countryCodesOnly_returnsFalse() {
        assertThat(BundledDataRefreshTool.isHelpRequested(new String[] {"CZ", "BE"})).isFalse();
    }

    @Test
    void isHelpRequested_noArgs_returnsFalse() {
        assertThat(BundledDataRefreshTool.isHelpRequested(new String[0])).isFalse();
    }
    ...
    void resolveCountryCodes_noArgs_returnsAllSupportedCountriesSorted() {
        List<String> countryCodes = BundledDataRefreshTool.resolveCountryCodes(new String[0]);

        assertThat(countryCodes).isSorted()
                                 .containsExactlyInAnyOrderElementsOf(BankDataRegistry.getSupportedCountryCodes());
    }

    @Test
    void resolveCountryCodes_validLowercaseCodes_returnsUppercased() {
        List<String> countryCodes = BundledDataRefreshTool.resolveCountryCodes(new String[] {"cz", "be"});
    ...
    void resolveCountryCodes_unknownCode_returnsNull() {
        List<String> countryCodes = BundledDataRefreshTool.resolveCountryCodes(new String[] {"CZ", "XX"});

        assertThat(countryCodes).isNull();
    }
